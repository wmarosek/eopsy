...
void initializePhilosphers(){
    ...
    if(SEMID < 0) {                           // checking if semaphores were created successfully
    ...
    for(i = 0; i < PHILOSOPHER_NO; i++){                            //  initialize all semaphores to 1 - all forks are free initially
        philosophersIds[i] = i;                                     // setting philosophersIds
...
void initializeThreads(){
    ...
    {
        pthread_create(&threadIds[i], NULL, philosopher, &philosophersIds[i]);  // create philosopher processes
    }

    // Join all threads
    for (i = 0; i < PHILOSOPHER_NO; i++)
    {
        pthread_join(threadIds[i], NULL);                               // wait equivalent
    }
